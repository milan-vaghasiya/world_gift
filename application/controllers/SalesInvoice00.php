...
class SalesInvoice extends MY_Controller{	
            ...
            if(!empty($data['item_id'])):
				...
				foreach($data['item_id'] as $key=>$value):
					...
					if(empty($data['location_id'][$key])):
						$errorMessage["qty".$i] = "Location is required.";
					elseif($data['stock_eff'][$key] == 1):
						...
						if(($currentStock + $old_qty) < $data['qty'][$key]):
					        $errorMessage["qty".$i] = "Stock not available.".$value;
						endif;
					endif;
					$i++;
				endforeach;
	...
	public function invoice_pdf()
	{
		...
		foreach ($expenseList as $row) {
			...
			if ($expAmt <> 0) {
				if ($row->position == 1) {
					$beforExp .= '<td colspan="2" class="text-right" style="border-top:1px solid #000;border-right:1px solid #000;">' . $row->exp_name.'</td>
	...
	public function sendPurchaseInvoice($id){
		...
		if(!empty($partyData)):
			...
			$trans_no = $this->db->select('ifnull((MAX(trans_no) + 1),1) as trans_no')->where('entry_type',12)->where('is_delete',0)->where('cm_id',$partyData->cm_id)->get('trans_main')->row()->trans_no;
