                            ...
                            <div class="col-md-5">  
                                <div class="input-group">
                                    ...
                                    <div class="input-group-append ml-2">
                                        <button type="button" data-file_type="" class="btn waves-effect waves-light btn-success float-right loaddata" title="Load Data">
									        <i class="fas fa-sync-alt"></i> Load
								        </button>
									<button type="button" data-file_type="PDF" id="pdf" name="pdf" class="btn waves-effect waves-light btn-primary float-right loaddata" title="Load Data"><i class="fa fa-print"></i> Print</a>
                            ...
                            <table id='commanTable' class="table table-bordered">
								<thead class="thead-info" id="theadData">
									...
									<tr>
										...
										<th style="min-width:50px;">Amount (Filtered Period)</th>
										<th style="min-width:50px;">Closing Amount</th>
									</tr>
								</thead>
								<tbody id="cashBookData"></tbody>
...
function loadData(file_type=''){
	...
	var postData= {from_date:from_date, to_date:to_date,file_type:file_type};
	if(valid){
		if(file_type == "")
		{
			$.ajax({
			url: base_url + controller + '/getCashBook',
			data: postData,
			type: "POST",
			dataType:'json',
		...
		else
		{
			...
			var url = base_url + controller + '/getCashBook/' + encodeURIComponent(u);
			window.open(url);
		}
	}
}
