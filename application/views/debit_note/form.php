		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<div class="card-body">
						<form autocomplete="off" id="saveDebitNote">
							<div class="col-md-12">
								<input type="hidden" name="id" value="<?= (!empty($invoiceData->id)) ? $invoiceData->id : "" ?>" />

								<input type="hidden" name="entry_type" id="entry_type" value="14">
								...
								<input type="hidden" name="reference_id" id="reference_id" value="<?= (!empty($invoiceData->ref_id)) ? $invoiceData->ref_id : $ref_id ?>">

								<input type="hidden" name="gst_type" id="gst_type" value="<?= (!empty($invoiceData->gst_type)) ? $invoiceData->gst_type : "" ?>">
								<input type="hidden" name="doc_no" id="doc_no" value="<?= (!empty($invoiceData->doc_no)) ? $invoiceData->doc_no : "" ?>">
								<input type="hidden" name="doc_date" id="doc_date" value="<?= (!empty($invoiceData->doc_date)) ? $invoiceData->doc_date : "" ?>">
								<input type="hidden" name="challan_no" id="challan_no" value="">


								<div class="row form-group">
									<div class="col-md-2">
										<label for="trans_no">Debitnote No.</label>
										<div class="input-group">
											<input type="text" name="trans_prefix" id="trans_prefix" class="form-control req" readonly value="<?= (!empty($invoiceData->trans_prefix)) ? $invoiceData->trans_prefix : $trans_prefix ?>" />

											<input type="text" name="trans_no" id="trans_no" class="form-control" readonly placeholder="Enter Invoice No." value="<?= (!empty($invoiceData->trans_no)) ? $invoiceData->trans_no : $nextTransNo ?>" />
									...
									<div class="col-md-2">
										...
										<input type="date" id="trans_date" name="trans_date" class=" form-control req trans_date" placeholder="dd-mm-yyyy" value="<?= (!empty($invoiceData->trans_date)) ? $invoiceData->trans_date : date("Y-m-d") ?>" />
									</div>
									<div class="col-md-3">
									...
									<div class="col-md-5">
										...
										<!-- <div for="party_id1" class="float-right">
											<a href="javascript:void(0)" class="text-primary font-bold createPurchaseInvoice permission-write1" datatip="Grn" flow="down">+ Purchase Invoice</a>
										</div> -->
								...
								<div class="row form-group">
									...
									<input type="hidden" name="challan_no" class="form-control" value="<?= (!empty($invoiceData->challan_no)) ? $invoiceData->challan_no : "" ?>" />
									...
									<div class="col-md-4 form-group">
										<label for="invoice_ids">Purchase Invoice</label>
										<select id="invoice_ids" class="form-control jp_multiselect" data-input_id="pi_id" multiple="multiple"></select>
					...
					<div class="card-footer">
						<div class="col-md-12">
							<button type="button" class="btn waves-effect waves-light btn-outline-success float-right save-form" onclick="saveDebitNote('saveDebitNote');"><i class="fa fa-check"></i> Save</button>
					...
					<div class="col-md-12">
						<div class="row form-group">
							...
							<div class="col-md-9 form-group">
								...
								<select name="item_id" id="item_id" class="form-control itemOptions req">
							...
							<div class="col-md-5 form-group stock_eff_div">
								<label for="location_id">Store Location</label>
								<select name="location_id" id="location_id" class="form-control select2 req">
									<option value='' data-store_name=''>Select Store Location</option>
								</select>
							</div>

							<div class="col-md-4 form-group stock_eff_div">
								<label for="batch_no">Heat/Batch No.</label>
								<select id="batch_no" name="batch_no" class="form-control single-select req">
									<option value="" data-batch_qty="0">Select Batch No.</option>
								</select>
							</div>

							<div class="col-md-3 form-group stock_eff_div">
							...
							<div class="col-md-4 form-group">
								...
								<input type="text" name="disc_per" id="disc_per" value="0" class="form-control floatOnly">
							</div>
...
<div class="modal fade" id="orderModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg" role="document" style="min-width:70%;">
		<div class="modal-content animated slideDown">
			<div class="modal-header">
				<h4 class="modal-title" id="createInvoice">GRN List</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			</div>
							...
							<table class="table table-bordered">
								<thead class="thead-info">
									<tr>
										...
										<th class="text-center" style="width:15%;">Purchase No</th>
...
<script src="<?php echo base_url(); ?>assets/js/custom/debit-note.js?v=<?= time() ?>"></script>
...
if (!empty($orderItems)) :
	...
	foreach ($orderItems as $row) :
		...
		$row->stateCode = "";
		$row->net_amount = ($row->cgst_amt + $row->sgst_amt + $row->amount - $row->disc_amt);
...
<script>
	$(document).ready(function() {
		...
		$(".select2").select2({ dropdownParent: $('.select2').parent() });
	});
</script>