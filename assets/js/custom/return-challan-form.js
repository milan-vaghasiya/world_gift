$(document).ready(function(){
    ...
    $(document).on('click','.returnItem',function(){
        ...
        $.ajax({
            url: base_url + controller + "/getReturnItemTrans",
            type:'post',
            data:{item_id:dataRow.item_id,ref_id:dataRow.ref_id,trans_type:2},
            dataType:"json",
            success:function(data){
                $("#returnItemTable").dataTable().fnDestroy();
                $("#returnItemTableData").html("");				
				$("#returnItemTableData").html(data.resultHtml);
				returnTable();
            }
        });
    });
...
function saveReturnItem(frm){
    var fd = new FormData(frm);
    var qty = $("#qty").val();
    $.ajax({
		url: base_url + controller + '/saveReturnItem',
		data:fd,
		type: "POST",
		processData:false,
		contentType:false,
...
function trashReturnItem(id,qty,name='Record'){
    var send_data = { id:id };
	$.confirm({
		title: 'Confirm!',
		...
		buttons: {   
			ok: {
				...
				action: function(){
					$.ajax({
						url: base_url + controller + '/deleteReturnItem',
						data: send_data,
						type: "POST",
						dataType:"json",
						success:function(data)
...
function returnTable(){
	var returnTable = $('#returnItemTable').DataTable( {
		lengthChange: false,
		responsive: true,
		'stateSave':true,
		retrieve: true,
	...
	returnTable.buttons().container().appendTo( '#returnItemTable_wrapper .col-md-6:eq(0)' );
	return returnTable;
};