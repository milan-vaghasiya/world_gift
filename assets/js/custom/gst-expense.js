$(document).ready(function () {
	...
	$(document).on('change', '#item_id', function () {
		...
		if (item_id == "") {
			...
			$("#item_type").val('');
			$("#item_code").val('');
			$("#item_name").val(itemData.party_name);
			$("#item_desc").val('');
			$("#hsn_code").val(itemData.hsn_code);
			$("#gst_per").val(itemData.gst_per);
			$("#cessPer").val(itemData.cess_per);
			$("#price").val('');
			$("#unit_name").val('');
			$("#unit_id").val('');
		}
		...
		if (formData.item_id == "") {
			...
			if ($.inArray(formData.item_id, item_ids) >= 0 && formData.row_index == "") {
				...
				if (formData.qty == "" || formData.qty == "0" || formData.price == "" || formData.price == "0") {
					...
					formData.gst_per = (formData.gst_per != "")?formData.gst_per:0;
					formData.price = (formData.price != "")?formData.price:0;
					formData.cess_per = (formData.cess_per != "")?formData.cess_per:0;
					var amount = 0; var total = 0; var disc_amt = 0; var igst_amt = 0;cess_amt=0;
					...
					cess_per = parseFloat(formData.cess_per).toFixed(2);
					cess_amt = parseFloat((cess_per * amount) / 100).toFixed(2);

					net_amount = parseFloat(parseFloat(amount) + parseFloat(igst_amt) + parseFloat(cess_amt)).toFixed(2);
...
function saveGstExpense(formId) {

	var fd = $('#' + formId)[0];
	var formData = new FormData(fd);
	$.ajax({
		url: base_url + controller + '/saveGstExpense',
		data: formData,
		processData: false,
		contentType: false,
		type: "POST",
...
function AddRow(data) {
	...
	var cessPerInput = $("<input/>", { type: "hidden", name: "cessPer[]", value: data.cess_per });
