$(document).ready(function () {
	...
	$(document).on('click', '.createPurchaseInvoice', function () {
		...
		if (party_id != "" || party_id != 0) {
			$.ajax({
				url: base_url + 'debitNote/getPartyOrders',
				type: 'post',
				...
				success: function (data) {
					...
					$("#party_so").attr('action', base_url + 'debitNote/createInvoice');
					$("#btn-create").html('<i class="fa fa-check"></i> Create Invoice');
					$("#partyName").html(party_name);
					$(".modal-title").html("GRN [" + party_name + "]");
	...
	$(document).on('change keyup', '#party_id', function () {
		...
		if ($(this).val() != "") {
			...
			$("#party_state_code").val(stateCode);
			$.ajax({
				url: base_url + controller + '/getPurchaseInvoiceList',
	...
	$(document).on('change', '#item_id', function () {
		...
		if (item_id == "") {
			...
			$(".stock_eff_div").hide();
			$("#location_id").html('');
			$("#location_id").html("<option value='' data-store_name=''>Select Store Location</option>");
			$("#location_id").select2({ dropdownParent: $('#location_id').parent() });
			$("#batch_no").html('');
			...
			$("#price").val(parseFloat(itemData.price).toFixed(2));
			$("#unit_name").val(itemData.unit_name);
			$("#unit_id").val(itemData.unit_id);
		}
	});

	$(document).on('change','#stock_eff',function(){
		var stock_eff = $(this).val();
		...
		if(stock_eff == 1){
			...
			if(item_id == ""){
				...
				$.ajax({
					...
					type:'post',
					data:{item_id:item_id,location_id:""},
					dataType:'json',
	...
	$(document).on('change',"#location_id",function(){
		...
		if(location_id == ""){
			...
			var item_id = $("#item_id").val();
			$.ajax({
				url:base_url + controller + '/getItemLocationWiseBatch',
				type:'post',
				data:{item_id:item_id,location_id:location_id,batch_no:""},
				dataType:'json',
				success:function(response){
					$("#batch_no").html("");
					$("#batch_no").html(response.options);
					$("#batch_no").comboSelect();	
					$("#stock_qty").val(0);				
				}
			});
	...
	$(document).on('change','#batch_no',function(){
		var stock_qty = $('#batch_no :selected').data('stock_qty');
	...
	$(document).on('click', '.saveItem', function () {
		...
		if (formData.item_id == "") {
			...
			var valid = 1;
			if(formData.stock_eff == 1){
				if(formData.location_id == ""){$('.location_id').html('Location is required.'); valid=0;}
				if(formData.batch_no == ""){$('.batch_no').html("Batch No. is required.");valid=0;}
			}
			...
			if(valid == 1){
				...
				formData.batch_qty = parseFloat(formData.qty).toFixed(2);
		...
		$.ajax({
			...
			success:function(response){
				...
				$.ajax({
					url:base_url + controller + '/getItemLocationWiseBatch',
					type:'post',
					data:{item_id:item_id,location_id:location_id,batch_no:batch_no},
					dataType:'json',
					success:function(response){
						$("#batch_no").html("");
						$("#batch_no").html(response.options);
...
function saveDebitNote(formId) {

	var fd = $('#' + formId)[0];
	var formData = new FormData(fd);
	$.ajax({
		url: base_url + controller + '/saveDebitNote',
		data: formData,
		processData: false,
		contentType: false,
		type: "POST",
