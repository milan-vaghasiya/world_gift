        ...
        if(formData.item_id == ""){
			...
			if ($.inArray(formData.item_id,item_ids) >= 0) {
				...
				if(formData.qty == "" || formData.qty == "0" || formData.price == "" || formData.price == "0"){
					...
					if(parseFloat(formData.disc_amount) > 0){
					    total = parseFloat(parseFloat(formData.qty) * parseFloat(formData.price)).toFixed(2);
						disc_per = parseFloat((parseFloat(formData.disc_amount) * 100) / total).toFixed(2);
						amount = parseFloat(total - parseFloat(formData.disc_amount)).toFixed(2);
					}else{
...
function AddRow(data) {
	...
	var discAmtInput = $("<input/>",{type:"hidden",name:"disc_amount[]",value:data.disc_amount});
...
function claculateColumn()
{
	...
	var discAmtArr = $("input[name='disc_amount[]']").map(function(){return $(this).val();}).get();;
    var discAmtSum = 0;
